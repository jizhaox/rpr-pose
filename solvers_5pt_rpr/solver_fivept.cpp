...
#include <Eigen/Dense>
#include "common.h"
#include "solver_fivept_cayley.h"
...
// mex(['-I"' eigen_dir '"'],'-O','solver_fivept.cpp')

using namespace std;
using namespace Eigen;

Eigen::MatrixXcd solve_equation_fivept_cayley(double *input)
{
    const VectorXd data = Map<const VectorXd>(input, 20);
    Eigen::MatrixXcd sols = solver_fivept_cayley(data);
    return sols;
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
    ...
    if (mxGetM(prhs[0])*mxGetN(prhs[0])!=20)
        mexErrMsgTxt("the 1st parameter should be 20 by 1!");

    if (!mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("fivept:notDouble", "Input data must be type double.");
    }
    
    double *input = (double *)mxGetData(prhs[0]);
    
    Eigen::MatrixXcd sols;
    sols = solve_equation_fivept_cayley(input);    

    std::vector<Eigen::Matrix<double,3,1>> q_arr, t_arr;
    std::vector<Eigen::Matrix<double,3,3>> rotm;
    std::vector<Eigen::Vector3d> P1, P2;
    format_covert_5pt(input, P1, P2);
