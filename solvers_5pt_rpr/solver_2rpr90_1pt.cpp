...
#include <Eigen/Dense>
#include "common.h"
#include "solver_2rpr90_1pt_cayley.h"
...
// mex(['-I"' eigen_dir '"'],'-O','solver_2rpr90_1pt.cpp')

using namespace std;
using namespace Eigen;

Eigen::MatrixXcd solve_equation_2rpr90_1pt_cayley(double *input)
{
    const VectorXd data = Map<const VectorXd>(input, 28);
    Eigen::MatrixXcd sols = solver_2rpr90_1pt_cayley(data);
    return sols;
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
    ...
    if (mxGetM(prhs[0])*mxGetN(prhs[0])!=28)
        mexErrMsgTxt("the 1st parameter should be 28 by 1!");
